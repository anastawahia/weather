اريد دمج هذه الدالة داخل المشروع
لقراءه ملفات json من المجلد data/structured
...
# ======================================================
# Advanced JSON Loader
# ======================================================
def load_json_documents(file_path: Path, max_sections: int = 5000) -> List[Document]:
    """
    Load complex JSON files (nested, large, or structured) into Document objects.
    Handles legal, analytical, and hierarchical data with intelligent flattening.
    Automatically detects:
      - Section-based structures (documents, articles, schedules, sections)
      - Array of records (list of dicts)
      - Deeply nested data with recursion
    """
    docs: List[Document] = []
    ...
    try:
        ...
        print(f"[JSON] ❌ Failed to load {file_path.name}: {e}", flush=True)
        return docs

    # Helper: recursive flattener for complex JSON
    def _flatten_json(obj: Any, prefix: str = "", depth: int = 0, max_depth: int = 6) -> List[str]:
        """Recursively flatten nested JSON structures into readable text blocks."""
        results = []
        if depth > max_depth:
        ...
        if isinstance(obj, dict):
            for k, v in obj.items():
                ...
                results.extend(_flatten_json(v, new_prefix, depth + 1))
        elif isinstance(obj, list):
            for i, v in enumerate(obj[:100]):  # avoid explosion on huge lists
                new_prefix = f"{prefix}[{i}]"
                results.extend(_flatten_json(v, new_prefix, depth + 1))
            if len(obj) > 100:
                results.append(f"{prefix} ... [truncated {len(obj)-100} items]")
        else:
            val = str(obj).strip()
            if val:
    ...
    # ===============================
    # CASE 1: Legal or structured JSON
    # ===============================
    if isinstance(data, dict) and any(k.lower() in data for k in ["sections", "articles", "clauses", "schedules"]):
        # detect section-based structure
        container_key = next(k for k in data if k.lower() in ["sections", "articles", "clauses", "schedules"])
        sections = data.get(container_key, [])
        if isinstance(sections, list):
            for i, sec in enumerate(sections[:max_sections]):
                title = sec.get("title") or sec.get("heading") or f"{container_key[:-1].capitalize()} {i+1}"
                ...
                doc = Document(
                    ...
                    metadata={
                        ...
                        "document_title": data.get("document") or file_path.stem,
                        "section_title": title,
                        "section_index": i + 1,
                        "container_key": container_key,
                    },
                    doc_id=f"file://{file_path.resolve()}#{i+1}"
                )
                docs.append(doc)

    # ===============================
    # CASE 2: List of dicts (data records)
    # ===============================
    elif isinstance(data, list) and all(isinstance(x, dict) for x in data):
        df = pd.DataFrame(data)
        if not df.empty:
            text_md = df.to_markdown(index=False) if len(df) < 2000 else df.head(2000).to_markdown(index=False)
            row_dump = "\n".join(
                " ; ".join(f"{k}: {v}" for k, v in row.items() if str(v).strip())
                for _, row in df.head(200).iterrows()
            )
            text = (
                f"[JSON-TABLE] file={file_path.name} | rows={len(df)} | cols={len(df.columns)}\n\n"
                + text_md + "\n\n-- SAMPLE ROWS --\n" + row_dump
            )
    ...
    # ===============================
    # CASE 3: Deep nested / irregular JSON
    # ===============================
    else:
        ...
        if flattened_text.strip():
            docs.append(Document(
                ...
                metadata={
                    ...
                    "source": file_path.name,
                    "source_path": str(file_path),
                    "note": "flattened from irregular or nested JSON"
                }
            ))

    print(f"[JSON] ✅ Loaded {len(docs)} documents from {file_path.name}")
    return docs


# ======================================================
# Load all JSON files from STRUCTURED_DIR
# ======================================================
def load_all_json_docs(json_dir: Path) -> List[Document]:
    """Scan directory and load all JSON files using advanced loader."""
    all_docs: List[Document] = []
    for file in json_dir.glob("*.json"):
        all_docs.extend(load_json_documents(file))
    return all_docs
